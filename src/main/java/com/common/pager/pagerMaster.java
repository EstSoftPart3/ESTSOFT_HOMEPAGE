...
public class pagerMaster {
    ...
    public pagerMaster(int count, int curPage, int pageScale, int blockScale){
        curBlock = 1; // 현재 페이지 블록 번호
        this.curPage = curPage; // 현재 페이지 설정
        setPageScale(pageScale);//페이지당 게시물 수 세팅
        setBlockScale(blockScale);//페이지당 게시물 수 세팅
    ...
    // WHERE rn BETWEEN #{start} AND #{end}
	//  >> pageBegin번째 인덱스부터 pageScale만큼 게시물 가져오기
        // 시작번호 = (현재페이지-1)*페이지당 게시물수
        pageBegin = (curPage-1)*pageScale;
        // 끝번호 = > 가져올 게시물 수 
        pageEnd = pageScale;
    }
    ...
    public void setTotBlock() {
        // 전체 페이지 갯수 / 10
        // 91 / 10 => 9.1 => 10개
        totBlock = (int)Math.ceil(totPage / blockScale);
    }
	...
	public String toString() {
		return "pagerMaster [pageScale=" + pageScale + ", blockScale=" + blockScale + ", curPage=" + curPage
				+ ", prevPage=" + prevPage + ", nextPage=" + nextPage + ", totPage=" + totPage + ", totBlock="
				+ totBlock + ", curBlock=" + curBlock + ", prevBlock=" + prevBlock + ", nextBlock=" + nextBlock
				+ ", pageBegin=" + pageBegin + ", pageEnd=" + pageEnd + ", blockBegin=" + blockBegin + ", blockEnd="
				+ blockEnd + "]";
	}
