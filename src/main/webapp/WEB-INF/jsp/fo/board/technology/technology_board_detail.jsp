...
<body>
    ...
    <div class="container-xxl py-5 menu">
        <div class="container">
            ...
            <div class="text-center mx-auto wow fadeInUp" data-wow-delay="0.1s" style="max-width: 500px;">
                <h1 class="display-3" style="font-size:20px;">기술지원 게시판 내용보기 </h1>
            </div>
		...
		<br>
         ...
         <div class="col-12 text-center" style="padding-top:15px;">
         	 ...
         	 <button type="button" class="btn btn-secondary btn-sm" onclick="technologyBoardRplyInsertPage();">답글</button>
	         <button type="button" class="btn btn-secondary btn-sm" onclick="technologyUpdatePage();">수정</button>
	         <button type="button" class="btn btn-secondary btn-sm" onclick="technologyBoardDelete();">삭제</button>
         </div>
    </div>
    <!-- FAQs Start -->
    ...
    <script>
	   ...
	   function technologyBoardDetailData(brdSq) {
			...
			$.ajax({
		           type: "post",
		           url: "/eep/board/technology/technologyBoardDetailData.do",
		           data: {
		        	   brdSq : brdSq
		            },
		   ...
		   var brdWrtrSq = $('#brdWrtrSq').val();
	    	...
	    	if(emplySq == brdWrtrSq){
	    		...
	    		if(confirm('정말 삭제 하시겠습니까?')) {
				   ...
				   $.ajax({
			           type: "post",
			           url: "/eep/board/technology/technologyBoardDeleteData.do",
			           data: {
			        	   brdSq : brdSq,
			           },
			           success: function(data) {
	   ...
	   function technologyUpdatePage() {
	    	...
	    	if(emplySq == brdWrtrSq){
	    		location.href='/eep/board/technology/openTechnologyBoardUpdate.do?brdSq='+brdSq;
	    	}else{
	    		bootbox.alert({
		...
		function technologyBoardRplyInsertPage() {
			...
			var brdReRep = $('#brdReRep').val();
		   	...
		   	location.href='/eep/board/technology/openTechnologyBoardReplyInsert.do?brdSq='+brdSq+'&brdReRep='+brdReRep+'&brdReLev='+brdReLev+'&brdReOrd='+brdReOrd;
		}
